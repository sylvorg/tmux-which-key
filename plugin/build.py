...
class Macro(object):
    ...
    def __post_init__(self):
        ...
        ), "macro has invalid name: {}".format(self.name)
...
class MenuItem(object):
    ...
    def __post_init__(
        ...
        if not self.separator:
            ...
            assert self.key, "item {} is missing a keybinding".format(self.name)
        ...
        if self.menu:
            ...
            assert len(self.menu) > 0, "menu {} has no commands".format(self.name)
            # Assign an alphanumeric menu ID for use in tmux user options.
            self.menu_id = "".join(
                [c for c in self.name if c.isalnum() or c in ["-", "_"]]
            ).lower()
            self.command = "show-wk-menu #{{@wk_menu_{}}}".format(self.menu_id)
        elif self.macro:
...
class Config(object):
    def __init__(
        ...
        ), "command_alias_start_index must be at least 200"
        ...
        macros = [
            {"name": "show-wk-menu", "commands": ["#{@wk_cmd_show}"]},
            {
    ...
    def register_menu(self, name: str, id: str, items: List[dict]):
        ...
        assert (
            menu.id not in self.menu_ids
        ), "a menu was already registered with ID: {}".format(menu.id)
        ...
        for item in items:
            ...
            if item.menu:
                ...
                assert (
                    item.macro in self.macro_names
                ), "item {} has unknown macro: {}".format(item.name, item.macro)

            if item.key:
                assert (
                    item.key not in mapped_keys
                ), "keybinding {} was already registered in menu {}".format(
                    item.key, name
                )
...
def main() -> int:
    ...
    parser.add_argument("config_file", help="The plugin config file", type=Path)
    parser.add_argument(
        "output_file",
        help="The file where plugin init script will be written",
        type=Path,
    )
